...
import { FOLDER_IDS, MOOD_IDS, STICKER_IDS, type DiaryEntry, type DiarySettings, type EntryFolder, type EntryFont, type EntryMood, type EntrySticker } from "./types";
import { normalizeFolder, normalizeMood, normalizeSettings, normalizeStickers } from "./validation";
...
export function validateBackupPayload(candidate: unknown): BackupPayload {
  ...
  return { version: 1, createdAt: value.createdAt, settings: normalizeSettings(value.settings), entries: (value.entries as DiaryEntry[]).map((entry) => { const folder = normalizeFolder(entry.folder); return { ...entry, ...(entry.stickers === undefined ? {} : { stickers: normalizeStickers(entry.stickers) }), ...(entry.mood === undefined ? {} : { mood: normalizeMood(entry.mood) }), ...(folder ? { folder } : {}) }; }), files };
}
