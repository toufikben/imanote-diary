...
import { FOLDER_IDS, INK_COLORS, MOOD_IDS, STICKER_IDS, type DiaryEntry, type DiarySettings, type DrawingStroke, type EntryFolder, type EntryFont, type EntryMood, type EntrySticker, type InkColor } from "./types";
import { normalizeDrawing, normalizeFolder, normalizeInkColor, normalizeMood, normalizeSettings, normalizeStickers } from "./validation";
...
function isFolder(value: unknown): value is EntryFolder { return typeof value === "string" && FOLDER_IDS.includes(value as EntryFolder); }
function isInkColor(value: unknown): value is InkColor { return typeof value === "string" && INK_COLORS.includes(value as InkColor); }
function isDrawing(value: unknown): value is DrawingStroke[] { return normalizeDrawing(value) !== undefined; }
...
export function validateBackupPayload(candidate: unknown): BackupPayload {
  ...
  return { version: 1, createdAt: value.createdAt, settings: normalizeSettings(value.settings), entries: (value.entries as DiaryEntry[]).map((entry) => { const folder = normalizeFolder(entry.folder); const inkColor = normalizeInkColor(entry.inkColor); const drawing = normalizeDrawing(entry.drawing); return { ...entry, ...(entry.stickers === undefined ? {} : { stickers: normalizeStickers(entry.stickers) }), ...(entry.mood === undefined ? {} : { mood: normalizeMood(entry.mood) }), ...(folder ? { folder } : {}), ...(inkColor ? { inkColor } : {}), ...(drawing ? { drawing } : {}) }; }), files };
}
